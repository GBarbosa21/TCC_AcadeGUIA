...
def exercicio_info(page: ft.Page):
    """View que lista TODOS os exercícios do Catálogo para adicionar aos salvos."""
    ...
    def carregar_lista():
        ...
        # Busca TODOS (filtrados por texto/grupo)
        ...
        if not dados:
            ...
            for ex in dados:
                ...
                def acao_salvar(e, nome=ex.nome):
                    # Chama o controller para inverter o status (Salvar/Remover)
                    novo_status = ExerciciosSalvosController.alternar_status_favorito(nome)

                    if novo_status:
                        msg = f"{nome} salvo nos favoritos!"
                        cor = ft.colors.GREEN
                    else:
                        msg = f"{nome} removido dos favoritos."
                        cor = ft.colors.RED
                ...
                card = btn_info_exercicio(
                    ...
                    on_Save=acao_salvar  # Conecta o botão de salvar
                )
    ...
    txt_busca = ft.TextField(hint_text="Buscar...", expand=True, height=45, text_style=ft.TextStyle(size=16),
